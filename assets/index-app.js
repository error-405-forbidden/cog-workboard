...
let siteNotes=[],notesReady=false,notesHasData=false,noteBusy=false,noteProject=TAGS[0],notesUnsubscribe=null,notesGeneration=0,notesTimer=null,noteProjectPicked=false,renameOpen=false,renameBusy=false;
...
$('renameProjectBtn').addEventListener('click',()=>{
 ...
 if(renameOpen){$('renameProjectInput').value=labelTag(noteProject);$('renameProjectInput').focus();}
});
$('renameProjectCancel').addEventListener('click',closeRename);
$('renameProjectSave').addEventListener('click',async()=>{
 if(renameBusy||!notesReady)return;
 ...
 const newTag=canonicalProject(raw);
 if(newTag===noteProject){closeRename();return;}
 if(getNoteProjects().includes(newTag)){$('renameProjectError').textContent='その名前の案件はすでにあります。';$('renameProjectError').hidden=false;return;}
 const oldTag=noteProject;renameBusy=true;syncNoteControls();$('renameProjectError').hidden=true;
 try{
  await W.renameProject(db,siteNotes,oldTag,newTag);
  if(oldTag in seenActivity){seenActivity[newTag]=seenActivity[oldTag];delete seenActivity[oldTag];saveSeen();}
  noteProject=newTag;closeRename();renderNotes();
